import {puzzleInput} from '../../data/twelve.js';
...
class Graph {
	...
	static canAddSmallCave(path, will_be_new_path) {
		...
		// Since we update `path` for first step,
		// don't include that part we will slice off later when testing the cave
				...
				for (let connection of tail.connections) {
					if (isLowerCase(connection) && path.includes(connection)) {
						if (
							...
							// Note that for part one, we always arrive at this branch
							// since `visit_single_small_cave_twice` is false
							continue;
						}
					}
